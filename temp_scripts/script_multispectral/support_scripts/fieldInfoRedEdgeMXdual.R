fieldInfoRedEdgeMXdual <- function (mosaic, fieldShape, fun = "function(x, na.rm) c(mean_fun = mean(x, na.rm=na.rm), sd_fun = sd(x, na.rm = na.rm), min_fun = min(x, na.rm=na.rm), max_fun = max(x, na.rm=na.rm), median_fun = median(x,na.rm = na.rm))",
                                    ...
                                    n.core = NULL, projection = TRUE) 
{
  ...
  field_data<-fieldShape@data[rep(seq_len(nrow(fieldShape@data)), each = 5), ]
  metric <- rep(c("mean","sd","min","max","median"),dim(fieldShape@data)[1])
